...
printPuzzle (Puzzle _ status checked) =
  "Current status: " ++ concat (map printLetter status) ++ "Count: " ++ (show $ length checked)
  where
...
getRandomWord = do
  rawFile <- readFile "./words.txt"
  let wordSet = rawFile |> lines |> filter (\x -> length x > 3)
  randomIndex <- randomRIO (0, length wordSet - 1)
  return $ wordSet !! randomIndex

getStartingPuzzle :: IO Puzzle
getStartingPuzzle = getRandomWord >>= \x -> return $ Puzzle x (knowNothing $ length x) []
  where
...
fillGuess (Puzzle problem status checked) guess =
  Puzzle problem (fillStatus problem status guess) (guess : checked)
  where
